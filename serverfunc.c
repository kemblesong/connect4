 ...
 */
int
get_move(int client, c4_t board) {
	int c;
	int valid;
	char* message = "Enter column number: ";
	send_message(client, message);
	read(client, &c, sizeof(int));
	/* check that a move is possible */
	...
	while ((c<=0) || (c>WIDTH) || (board[HEIGHT-1][c-1]!=EMPTY)) {
		message = "That move is not possible. Enter column number: ";
		send_message(client, message);
		valid = 0;
		write(client, &valid, sizeof(int));
		read(client, &c, sizeof(int));
	}
 ...
 */
void
print_config(c4_t board, int client) {
	int r, c, i, j;
	char buffer[1024];
	...
	for (r=HEIGHT-1; r>=0; r--) {
		for (i=0; i<HGRID; i++) {
			...
			for (c=0; c<WIDTH; c++) {
				for (j=0; j<WGRID; j++) {
					sprintf(eos(buffer), "%c", board[r][c]);
				}
				sprintf(eos(buffer), "|");
			}
			sprintf(eos(buffer), "\n");
